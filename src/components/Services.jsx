         ...
         <div className='flex flex-col md:flex-row gap-4 mb-12'>
           ...
           <div className='md:w-2/3'>
            <p className='text-secondary md:w-2/3'>
              At our digital marketing agency, we offer a range of services to help, grow and succeed online. These services include:
            </p>
           </div>
         </div>
         ...
         <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            {
              servicesData.map((service,index)=>(
                <div key={index} className={`flex flex-col sm:flex-row md:justify-between md:items-center gap-6 p-6 rounded-lg border border-[#191A23] cursor-pointer ${service.backgroundColor} ${service.textColor}`}>
                   <div className='md:w-1/2 flex flex-col h-ull justify-between'>
                   ...
                   <div className='md:w-1/2 order-first md:order-last'>
                      <img src={service.image} alt="" className='w-full object-cover rounded-md mb-4'></img>
                   </div>
                </div>
              ))
            }
